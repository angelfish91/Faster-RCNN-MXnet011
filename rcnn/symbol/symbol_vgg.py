...
def get_vgg_test(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    # Fast R-CNN
    if config.TEST.ENABLE_DEFORMABLE_ROIPOOLING:
        conv_new_1 = mx.sym.Convolution(data = relu5_3, kernel=(1, 1), num_filter=512, name="conv_new_1")
        ...
        pool5 = mx.contrib.sym.DeformablePSROIPooling(name='deformable_roi_pool',
                  data=conv_new_1_relu, 
                  rois=rois,
...
def get_vgg_train(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    '''
    Preparation
    1. softmax output of RPN class score
    2. reshape it
    mxnet already implement the proposal symbol
    ...
    group = mx.symbol.Custom(rois=rois, gt_boxes=gt_boxes_reshape, 
    ...
    # Fast R-CNN
    if config.TRAIN.ENABLE_DEFORMABLE_ROIPOOLING:
        conv_new_1 = mx.sym.Convolution(data = relu5_3, kernel=(1, 1), num_filter=512, name="conv_new_1")
        conv_new_1_relu = mx.sym.Activation(data=conv_new_1, act_type='relu', name='conv_new_1_relu')
        offset_t = mx.contrib.sym.DeformablePSROIPooling(name='offset_t', 
                                         data=conv_new_1_relu, 
