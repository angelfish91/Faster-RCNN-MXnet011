...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch,
    ...
    logger.info('arg shape dict %s' % pprint.pformat(arg_shape_dict))
    ...
    if args.resume:
        ...
        if config.TRAIN.ENABLE_DEFORMABLE_ROIPOOLING:
            arg_params['conv_new_1_weight'] = mx.random.normal(0, 0.001, shape=arg_shape_dict['conv_new_1_weight'])
            arg_params['conv_new_1_bias'] = mx.nd.zeros(shape=arg_shape_dict['conv_new_1_bias']) 
            arg_params['offset_weight'] = mx.random.normal(0, 0.001, shape=arg_shape_dict['offset_weight'])
